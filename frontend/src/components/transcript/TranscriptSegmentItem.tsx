...
export const TranscriptSegmentItem: React.FC<TranscriptSegmentItemProps> = ({
      ...
      <>
        {parts.map((part, i) =>
          part.toLowerCase() === query.toLowerCase() ? (
            <mark
              key={i}
              className={`rounded px-0.5 ${isCurrentSearchResult ? 'bg-amber-400 text-slate-950 font-semibold' : 'bg-amber-400/25 text-amber-200'
                }`}
            >
              {part}
            </mark>
  ...
  return (
    <div
      ...
      className={`group relative px-3.5 py-2.5 rounded-lg border-l-2 transition-all duration-150 ${isActive
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <button
          ...
          className={`shrink-0 mono text-[11px] font-medium transition-colors pt-0.5 select-none ${isActive
        ...
        <div className="flex-1 min-w-0">
          ...
          {isEditing ? (
            ...
            <p
              onClick={() => onSeek(segment.start_time)}
              className={`text-xs sm:text-[13.5px] leading-relaxed cursor-pointer select-text transition-colors ${isActive ? 'text-slate-100 font-normal' : 'text-slate-300/90'
                }`}
            >
